...
function [G,gabout] = gaborfilter1(iname,Sx,Sy,f,theta);

  I = imread(iname);
  
  if (ismatrix (I) && ndims(I) == 2 && columns(I) == 3)
    ntscmap = rgb2ntsc (I);
    gray = ntscmap (:, 1) * ones (1, 3);
  elseif (ismatrix(I) && ndims(I) == 3)
    I = rgb2gray(I);
  endif
  ...
  for x = -fix(Sx):fix(Sx)
    for y = -fix(Sy):fix(Sy)
        ...
        G(fix(Sx)+x+1,fix(Sy)+y+1) = (1/(2*pi*Sx*Sy))*exp(-.5*((xPrime/Sx)^2+(yPrime/Sy)^2))*exp(2*pi*f*xPrime);
    end
  end
